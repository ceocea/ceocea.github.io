    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $interval, growl, growlMessages) {
          ...
          $scope.customIcon = function(backgroundImage) {
            return backgroundImage ? {'background-image' : 'url(/images/growl/' + backgroundImage + '.png?v=1)'} : '';
          }
        }
      ]
    };
...
angular.module('angular-growl').run([
  ...
  function ($templateCache) {
    ...
    if ($templateCache.get('templates/growl/growl.html') === undefined) {
      $templateCache.put('templates/growl/growl.html', '<div class="growl-container" ng-class="wrapperClasses()">' + '<div' + ' ng-style="customIcon(message.backgroundImage)" ' + 'class="growl-item alert" ng-repeat="message in growlMessages.directives[referenceId].messages" ng-class="alertClasses(message)" ng-click="stopTimeoutClose(message)">' + '<button type="button" class="close" data-dismiss="alert" aria-hidden="true" ng-click="growlMessages.deleteMessage(message)" ng-show="!message.disableCloseButton">&times;</button>' + '<button type="button" class="close" aria-hidden="true" ng-show="showCountDown(message)">{{message.countdown}}</button>' + '<h4 class="growl-title" ng-show="message.title" ng-bind="message.title"></h4>' + '<div class="growl-message" ng-bind-html="message.text"></div>' + '</div>' + '</div>');
    }
  }
]);
    ...
    function ($rootScope, $interpolate, $sce, $filter, $interval, growlMessages) {
      ...
      function sendMessage(text, config, severity) {
        ...
        message = {
          ...
          backgroundImage: _config.backgroundImage || _backgroundImage,
